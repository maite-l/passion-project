...
void main() {
    ...
    //STAR/CIRCLES
    // 3 star shapes from gradients, with offset in rotation, position, line thickness and gradient
    vec2 st1 = st + vec2(-0.15f, -0.15f) + (noise(u_time) * 2.f - 1.f) / 100.f;
    st1 = rotate(st1, cos(u_time));
    ...
    vec2 st2 = st + vec2(0.15f, 0.f) + (noise(u_time + 2.f) * 2.f - 1.f) / 100.f;
    st2 = rotate(st2, cos(u_time + 2.f));
    ...
    vec2 st3 = st + vec2(-0.1f, 0.15f) + (noise(u_time + 3.f) * 2.f - 1.f) / 100.f;
    st3 = rotate(st3, cos(u_time + 3.f));
