...
import fragmentShaderPPRaw from './shaders/frag_post-processing.glsl?raw';
import vertexShaderPPRaw from './shaders/vert_post-processing.glsl?raw';
...
const initShaders = (canvas, fs, gl, ctx) => {
    ...
    const program1 = createProgram(gl, vertexShaderRaw, fs);
    ...
    const program2 = createProgram(gl, vertexShaderPPRaw, fragmentShaderPPRaw);
...
const init = () => {
    ...
    canvases1.forEach(canvas => {
        ...
        initShaders(canvas, fragmentShaderRaw1, gl, ctx);
    });
    canvases2.forEach(canvas => {
        ...
        const gl = canvas.getContext('webgl2');
        const ctx = canvasV.getContext('2d');
        initShaders(canvas, fragmentShaderRaw2, gl, ctx);
    });
    canvases3.forEach(canvas => {
        ...
        const gl = canvas.getContext('webgl2');
        const ctx = canvasV.getContext('2d');
        initShaders(canvas, fragmentShaderRaw3, gl, ctx);
    });
